...
const resolveAvatar = (avatar) => avatar && avatar.trim().length > 0 ? avatar : DEFAULT_AVATAR_PATH;
...
async function startServer() {
    ...
    // MongoDB Connection
    const MONGODB_URI = process.env.MONGODB_URI || "mongodb://localhost:27017/nexen";
    try {
    ...
    const serializePost = async (post) => {
        ...
        const commentUsersMap = new Map(commentUsers.map((u) => [u._id.toString(), serializeUser(u)]));
        return {
            id: post._id,
            userId: post.userId,
            user: author
            ...
            commentList: comments.map((comment) => ({
                ...
                user: commentUsersMap.get(comment.userId) ||
                    fallbackUser(comment.userId || "unknown-comment-user", "Deleted User"),
                text: comment.text,
                parentCommentId: comment.parentCommentId,
                timestamp: new Date(comment.timestamp || post.createdAt || new Date()).toISOString(),
            })),
            likedByUsers,
    ...
    app.put("/api/posts/:postId", authenticateToken, async (req, res) => {
        try {
            ...
            post.tags =
                nextContent.match(/#\w+/g)?.map((t) => t.slice(1)) || [];
            await post.save();
    ...
    // Reply to a comment
    app.post("/api/posts/:postId/comments/:commentId/reply", authenticateToken, async (req, res) => {
        try {
            ...
            const parentComment = post.commentList.find((c) => c._id?.toString() === commentId);
            if (!parentComment) {
    ...
    app.put("/api/posts/:postId/comments/:commentId", authenticateToken, async (req, res) => {
        try {
            ...
            const comment = post.commentList.find((c) => c._id?.toString() === commentId);
            if (!comment) {
                return res.status(404).json({ error: "Comment not found" });
            }
            // Check ownership
    ...
    app.delete("/api/posts/:postId/comments/:commentId", authenticateToken, async (req, res) => {
        try {
            ...
            const commentIndex = post.commentList.findIndex((c) => c._id?.toString() === commentId);
            if (commentIndex === -1) {
                return res.status(404).json({ error: "Comment not found" });
            }
            ...
            post.commentList = post.commentList.filter((c, idx) => idx === commentIndex ? false : c.parentCommentId !== commentId);
            post.comments = post.commentList.length;
            await post.save();
            res.json(await serializePost(post));
        }
